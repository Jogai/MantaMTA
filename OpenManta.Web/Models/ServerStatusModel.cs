...
namespace WebInterface.Models
{
    ...
    {
        public ServerStatusDirectoryInfo QueueDir =
            new ServerStatusDirectoryInfo(OpenManta.Framework.MtaParameters.MTA_QUEUEFOLDER);

        public ServerStatusDirectoryInfo LogDir =
            new ServerStatusDirectoryInfo(OpenManta.Framework.MtaParameters.MTA_SMTP_LOGFOLDER);

        public ServerStatusDirectoryInfo DropDir =
            new ServerStatusDirectoryInfo(OpenManta.Framework.MtaParameters.MTA_DROPFOLDER);
        ...
        public ServerStatusModel()
        {
            ...
            var logDir = ConfigurationManager.AppSettings["MantaResetMtaLogPath"];
            if (logDir.StartsWith("~"))
            {
                logDir = logDir.Replace("~", AppDomain.CurrentDomain.BaseDirectory);
            }
            if (!File.Exists(logDir))
            {
                File.Create(logDir).Dispose();
            }
            using (StreamReader sr = new StreamReader(logDir))
            {
                while (sr.Peek() != -1)
                {
                    if (lines.Count == 20)
    ...
    public class ServerStatusDirectoryInfo
    {
        ...
        public string FreeSpace
        {
            get
            {
                ...
                long freeSpace = DriveInfo.GetDrives()
                    .Single(d => d.RootDirectory.ToString().Equals(pathRoot, StringComparison.OrdinalIgnoreCase))
                    .TotalFreeSpace;
                if (freeSpace < 1024)
                    return "0 Kb";
